...
let shootingArray = new Array(); // krakac elementneri masiv@
...
function setNearDisabledForTwo(eventTargetElement, pointerEvents,  type = true){  //ete tayp@ true e uremn x@ drvac e
    ...
    // eventTargetElement.classList.add('two')

    // eventTargetElement.dataset.bb = 'ddd';
...
function shootComputerCloseFunction( row, column ){
    ...
    addElementsToDynamicArray(row, column);

    let shipNotDie = dinamicArr.some((item) => {
        return document.getElementById(item).textContent == 'x';  // ete gone mi hati textcontent@ == x kanchum e  shootComputerDynamic functian
    })

    if(shipNotDie){
        shootComputerDynamic(); //merac chi
    } else {
        console.log("playTableItems");
        console.log(playTableItems);
        dinamicArr.forEach((item)=>{
            playTableItems.splice(playTableItems.indexOf(item), 1);
        })
        console.log("after playTableItems");
        console.log(playTableItems);
        dinamicArr = [];
        shootComputerFunction();
    }
...
function shootComputerDynamic() {
    let columnDynamic;
    let letterDynamic;
    let dynamicRandomId = Math.floor(Math.random()*dinamicArr.length);

    let shootDynamicItem = dinamicArr[dynamicRandomId];
    [ letterDynamic , columnDynamic] = shootDynamicItem.split('-');
    dinamicArr.splice(dinamicArr.indexOf(shootDynamicItem), 1);
    console.log("splice dynamic element")
    console.log(dinamicArr);

    //addElementsToDynamicArray(letterDynamic, columnDynamic);

    let dynamicElement = document.getElementById(shootDynamicItem);

    console.log("dynamicElement:" + shootDynamicItem);
    console.log(dynamicElement);
    if(dynamicElement.textContent != ''){
        dynamicElement.classList.add('shoot');
        shootingArray.push(shootDynamicItem);  // cisht xpac elementneri masiv aysinqn ete length@ == 20 uremn hamakargich@ krel e
        playTableItems.splice(playTableItems.indexOf(shootDynamicItem), 1);

        setTimeout(() => {
            shootComputerCloseFunction(letterDynamic, columnDynamic);
        }, 500);


    } else {
        dynamicElement.classList.add('shoot-empty');
        shootingArray.push(dynamicRandomId);
        playTableItems.splice(playTableItems.indexOf(shootDynamicItem), 1);
    }
}

function addElementsToDynamicArray(letter, column) {
    let elementShoot = document.getElementById(letter + '-' + column);

    let elementLeftId = elementShoot?.previousElementSibling;
    let elementRightId = elementShoot?.nextElementSibling;

    let elementTopRow = elementShoot?.dataset.prevrow;
    let elementBottomRow = elementShoot?.dataset.nextrow;

    if(elementLeftId && elementLeftId.tagName != 'TH') {
        if(!elementLeftId.classList.contains('shoot') && !elementLeftId.classList.contains('shoot-empty')) {
            dinamicArr.push(elementLeftId.id);
        }
    }

    if(elementRightId) {
        if(!elementRightId.classList.contains('shoot') && !elementRightId.classList.contains('shoot-empty')) {
            dinamicArr.push(elementRightId.id);
        }
    }

    if(elementTopRow) {
        let elementTopRowId = document.getElementById(elementTopRow +'-'+ column); // verevi tox@
        if(!elementTopRowId.classList.contains('shoot') && !elementTopRowId.classList.contains('shoot-empty')) {
            dinamicArr.push(elementTopRow +'-'+ column);
        }
    }
    if(elementBottomRow) {
        let elementBottomRowId = document.getElementById(elementBottomRow +'-'+ column);  // nerqevi tox@
        if(!elementBottomRowId.classList.contains('shoot') && !elementBottomRowId.classList.contains('shoot-empty')) {
            dinamicArr.push(elementBottomRow +'-'+ column);
        }
